  ...
  <div style="display: flex; flex: 1; overflow: hidden">
    <div style="padding: 25px; padding-right: 0; width: 250px; flex-shrink: 0; overflow: hidden">
      <div style="
        ...
        ">
        <el-tree  :props="treeProps" :expand-on-click-node="false"
      ...
      <admin-table @init="(table) => ($table = table)" :api="listUser">
          ...
          <el-form ref="queryFrom" @submit.prevent="$table.search" label-width="auto" :model="$table.query">
            <admin-grid :cols="4" :x-gap="20">
              ...
              <el-form-item label="审核">
                <select-dict :dict="$dict.audit_status_search" v-model="$table.query.auditStatus" show-all></select-dict>
              </el-form-item>
        ...
        <template #table>
          <el-table :data="$table.data" style="width: 100%" :empty-text="$table.emptyText" v-loading="$table.loading"
            ...
            <el-table-column label="审核" prop="auditStatus"  :formatter="$formatter.dict($dict.audit_status)"  />
            ...
            <el-table-column label="操作" width="300">
              <template #default="scope">
                <el-space spacer="|" v-if="scope.row.userId !== 1">
                  ...
                  <el-link v-if="$p(['system:user:audit'])" type="primary" @click="handleAudit(scope.row)">
                    审核
                  </el-link>
                </el-space>
              </template>
...
getDict(['sys_normal_disable', 'sys_user_sex','audit_status_search'])
...
const treeProps=ref({ label: 'label',
 children: 'children' ,
 isLeaf: 'leaf' // 可选，用于优化展示判断

})
const loadNode = async (node, resolve) => {
  if (node.level === 0) {
    // 根节点，加载所有顶级部门
    const res = await deptTreeSelectAsync(0)
    resolve(res.data)
  } else {
    const parentId = node.data.id
    const res = await deptTreeSelectAsync(parentId)
    resolve(res.data)
  }
}
/** 节点单击事件 */
  ...
  adminDialog(
    {
      ...
      dialogProps: { title: '用户审核', width: '800px' }
    }
  )
}
