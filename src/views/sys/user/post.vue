<template>
  <admin-dialog-content>
    <el-form ref="postForm" label-position="right" label-width="130px" :model="postData">
      ...
      <admin-space>
        ...
        <el-form-item label="所属地区/部门" prop="deptId">
          <!--el-tree-select v-model="postData.deptId" :data="deptOptions"
            ...
            <select-dept v-model="postData.deptId"  :multiple="false" :checkStrictly="true"></select-dept>
        </el-form-item>
         <el-form-item label="账号" prop="userName" :readonly="postData.userId!=null" :rules="[$rules.required]">
          <el-input v-model="postData.userName"></el-input>
        </el-form-item>
        <el-form-item label="手机号码" prop="phonenumber" :rules="[$rules.required]">
          <el-input v-model="postData.phonenumber" :readonly="postData.userId!=null"></el-input>
...
const submitForm = async () => {
  ...
  try {
    await checkRules(postForm)
     var post = {...postData.value};
     post.deptId = Array.isArray(post.deptId)?post.deptId[post.deptId.length-1]:post.deptId;
