<template>
    <div class="admin-view">
        <admin-table :api="list" @init="(table) => { $table = table }">
            ...
            <template #btn>
                <el-button type="danger" plain icon="Delete" :disabled="$table.selection.length == 0"
                    @click="handleDel()" v-if="$p(['monitor:operlog:remove'])">删除</el-button>
    ...
    adminDialog(
        {
            ...
            dialogProps: { title: '操作日志详细', width: "800px" }
        }
    )
}
</script>