<template>
  <admin-dialog-content>
    <admin-table :api="unallocatedUserList" @init="(table) => {
      $table = table
    }
      " :query="props.query">
      <template #search>
      ...
      <template #table>
        <el-table :data="$table.data" table-layout="auto" style="width: 100%" :empty-text="$table.emptyText"
          ...
          <el-table-column label="状态" :formatter="$formatter.dict($dict.sys_normal_disable)"
...
function submitForm() {
  ...
  authUserSelectAll({
    roleId: roleId,
    userIds: $table.selection.map((item) => item.userId)?.join(',')
  })
    .then((res) => {
