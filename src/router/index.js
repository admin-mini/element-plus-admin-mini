...
export const constantRoutes = [
  ...
  {
    path: '/:pathMatch(.*)*',
    component: () => import('@/layout/other/page-404.vue'),
    meta: { noTag: true },
    hidden: true
  },
  {
    path: '/401',
    component: () => import('@/layout/other/page-401.vue'),
    meta: { noTag: true },
    hidden: true
  },
  // {
...
router.beforeEach(async (to, from, next) => {
  ...
  if (permissionStore.addRoutes.length == 0) {
    try {
      ...
      next(router.resolve({ ...to, replace: true }))
    } catch (err) {
      ElMessage.error('获取路由失败，请稍后再试')
      next(router.resolve({ name: 'login', replace: true }))
    }
  }
